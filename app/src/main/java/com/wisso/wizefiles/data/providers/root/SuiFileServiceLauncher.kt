...
object SuiFileServiceLauncher {
    ...
    fun launchService(): IFileServiceBridge {
        synchronized(lock) {
            if (!isSuiAvailable()) {
                throw BridgeUnavailableException("Sui isn't available")
            }
            if (Shizuku.checkSelfPermission() != PackageManager.PERMISSION_GRANTED) {
                ...
                if (!granted) {
                    throw BridgeUnavailableException("Sui permission isn't granted")
                }
            }
            return try {
                runBlocking {
                            ...
                            suspendCancellableCoroutine { continuation ->
                                ...
                                val connection = object : ServiceConnection {
                                    ...
                                    override fun onBindingDied(name: ComponentName) {
                                        if (continuation.isActive) {
                                            continuation.resumeWithException(
                                                BridgeUnavailableException("Sui binding died")
                                            )
                                        }
                                    }
                                    ...
                                    override fun onNullBinding(name: ComponentName) {
                                        if (continuation.isActive) {
                                            continuation.resumeWithException(
                                                BridgeUnavailableException("Sui binding is null")
                                            )
                                        }
                                    }
                                }

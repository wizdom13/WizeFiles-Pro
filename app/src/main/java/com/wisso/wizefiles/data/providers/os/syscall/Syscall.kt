...
import com.wisso.wizefiles.provider.common.moveToByteString
import com.wisso.wizefiles.core.selinux.SeLinuxBridge
...
object Syscall {
    ...
    fun getfilecon(path: ByteString): ByteString =
        try {
            SeLinuxBridge.getFileContext(path.borrowBytes()).moveToByteString()
        } catch (e: ErrnoException) {
            throw SyscallException(e)
        }
    ...
    fun is_selinux_enabled(): Boolean = SeLinuxBridge.isSelinuxEnabled()
    ...
    fun lgetfilecon(path: ByteString): ByteString =
        try {
            SeLinuxBridge.lGetFileContext(path.borrowBytes()).moveToByteString()
        } catch (e: ErrnoException) {
            throw SyscallException(e)
        }
    ...
    fun lsetfilecon(path: ByteString, context: ByteString) {
        try {
            SeLinuxBridge.lSetFileContext(path.borrowBytes(), context.borrowBytes())
        } catch (e: ErrnoException) {
            throw SyscallException(e)
        }
    }
    ...
    fun selinux_android_restorecon(path: ByteString, flags: Int) {
        try {
            SeLinuxBridge.restoreContext(path.borrowBytes(), flags)
        } catch (e: ErrnoException) {
            throw SyscallException(e)
        }
    }
    ...
    fun setfilecon(path: ByteString, context: ByteString) {
        try {
            SeLinuxBridge.setFileContext(path.borrowBytes(), context.borrowBytes())
        } catch (e: ErrnoException) {
            throw SyscallException(e)
        }
    }

    ...
    private class Poller(
        ...
        private fun maybeAddDontFollowMask(path: Path, mask: Int): Int {
            ...
            return if (attributes != null && attributes.isSymbolicLink) {
                mask or LinuxSyscallConstants.IN_DONT_FOLLOW
            } else {
                mask
            }
        }
                            ...
                            try {
                                if (key.isValid) {
                                    ...
                                    try {
                                        ...
                                        com.wisso.wizefiles.util.AppLog.e("Error", "Unexpected failure", e.toFileSystemException(key.watchable().toString()))
                                    }
                                    key.setInvalid()
                                    keys.remove(wd)
                                }
                ...
                while (true) {
                    ...
                    if (fds[1].revents.toInt().hasBits(OsConstants.POLLIN)) {
                        ...
                        if (size > 0) {
                            ...
                            for (event in events) {
                                if (event.mask.hasBits(LinuxSyscallConstants.IN_Q_OVERFLOW)) {
                                    for (key in keys.values) {
                                        key.addEvent(StandardWatchEventKinds.OVERFLOW, null)
                                    }
                                    break
                                ...
                                if (event.mask.hasBits(LinuxSyscallConstants.IN_IGNORED)) {
                                    key.setInvalid()
                                    key.signal()
            ...
            for (kind in kinds) {
                when (kind) {
                    StandardWatchEventKinds.ENTRY_CREATE ->
                        mask = mask or (LinuxSyscallConstants.IN_CREATE or LinuxSyscallConstants.IN_MOVED_TO)
                    StandardWatchEventKinds.ENTRY_DELETE ->
                        mask = mask or (LinuxSyscallConstants.IN_DELETE_SELF or LinuxSyscallConstants.IN_DELETE
                            or LinuxSyscallConstants.IN_MOVED_FROM)
                    StandardWatchEventKinds.ENTRY_MODIFY ->
                        mask = mask or (LinuxSyscallConstants.IN_MOVE_SELF or LinuxSyscallConstants.IN_MODIFY
                            or LinuxSyscallConstants.IN_ATTRIB)
                }
            }
            return mask
        }
        ...
        private fun maskToEventKind(mask: Int): WatchEvent.Kind<Path> =
            when {
                mask.hasBits(LinuxSyscallConstants.IN_CREATE) || mask.hasBits(LinuxSyscallConstants.IN_MOVED_TO) ->
                    StandardWatchEventKinds.ENTRY_CREATE
                mask.hasBits(LinuxSyscallConstants.IN_DELETE_SELF) || mask.hasBits(LinuxSyscallConstants.IN_DELETE)
                    || mask.hasBits(LinuxSyscallConstants.IN_MOVED_FROM) ->
                    StandardWatchEventKinds.ENTRY_DELETE
                mask.hasBits(LinuxSyscallConstants.IN_MOVE_SELF) || mask.hasBits(LinuxSyscallConstants.IN_MODIFY)
                    || mask.hasBits(LinuxSyscallConstants.IN_ATTRIB) -> StandardWatchEventKinds.ENTRY_MODIFY

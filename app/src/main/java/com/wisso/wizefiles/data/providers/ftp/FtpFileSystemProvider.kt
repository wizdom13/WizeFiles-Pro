...
import com.wisso.wizefiles.provider.common.DelegateSchemeFileSystemProvider
...
object FtpFileSystemProvider : FileSystemProvider(), PathObservableProvider, Searchable {
    ...
    override fun newInputStream(file: Path, vararg options: OpenOption): InputStream {
        ...
        if (openOptions.create || openOptions.createNew || openOptions.noFollowLinks) {
            val fileFile = try {
                FtpClient.listFileOrNull(file, true)
            } catch (e: IOException) {
        ...
        try {
            return FtpClient.retrieveFile(file)
        } catch (e: IOException) {
            throw e.toFileSystemExceptionForFtp(file.toString())
        }
    }
    ...
    override fun newOutputStream(file: Path, vararg options: OpenOption): OutputStream {
        ...
        try {
            return FtpClient.storeFile(file)
        } catch (e: IOException) {
            throw e.toFileSystemExceptionForFtp(file.toString())
        }
    }
    ...
    override fun newByteChannel(
        ...
        try {
            return FtpClient.openByteChannel(file, openOptions.append)
        } catch (e: IOException) {
            throw e.toFileSystemExceptionForFtp(file.toString())
        }
    }
    ...
    override fun newDirectoryStream(
        ...
        val paths = try {
            ...
            FtpClient.listDirectory(directory) as List<Path>
        } catch (e: IOException) {

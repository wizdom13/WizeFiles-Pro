...
import com.wisso.wizefiles.core.android.compat.getFrameAtTimeCompat
import com.wisso.wizefiles.core.android.compat.getScaledFrameAtTimeCompat
...
class VideoFrameFetcher(
    ...
    override suspend fun fetch(): FetchResult =
        MediaMetadataRetriever().use { retriever ->
            ...
            val decodedBitmap = outBitmap
                ?: throw IOException("Failed to decode video frame at ${frameMicros} microseconds")
            val scale = if (options.allowInexactSize) {
                rawScale.coerceAtMost(1.0)
            } else {
                rawScale
            ...
            val isValidSize = if (options.allowInexactSize) {
                decodedBitmap.width <= width && decodedBitmap.height <= height
            } else {
                decodedBitmap.width == width && decodedBitmap.height == height
            }
            val isValidConfig = decodedBitmap.config?.isHardware != true || options.config.isHardware
            val bitmap = if (isValidSize && isValidConfig) {
                ...
                createBitmap(width, height, config).applyCanvas {
                    ...
                    drawBitmap(decodedBitmap, 0f, 0f, paint)

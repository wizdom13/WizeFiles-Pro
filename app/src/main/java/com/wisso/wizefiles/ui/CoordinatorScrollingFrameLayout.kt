...
import com.wisso.wizefiles.util.layoutInNavigation

class CoordinatorScrollingFrameLayout : FrameLayout, AttachedBehavior {
    private var bottomInsets: Insets? = null
    ...
    override fun onApplyWindowInsets(insets: WindowInsets): WindowInsets {
        ...
        bottomInsets = Insets.of(0, 0, 0, systemBarsInsets.bottom)
        requestLayout()
        return insets
    }
                    ...
                    if (scrollingView.fitsSystemWindows) {
                        ViewCompat.dispatchApplyWindowInsets(
                            ...
                            WindowInsetsCompat.Builder()
                                .setInsets(WindowInsetsCompat.Type.systemBars(), bottomInsets)
                                .build()
                        )
                    } else {
                        scrollingView.updatePadding(bottom = bottomInsets.bottom)
                    }
                } else {
    ...
    private class Behavior : ScrollingViewBehavior() {
        override fun onMeasureChild(
            ...
            if (parentInsets != null) {
                val systemBarsInsets = parentInsets.getInsets(WindowInsetsCompat.Type.systemBars())
                val parentHeightSize = (MeasureSpec.getSize(parentHeightMeasureSpec)
                    - systemBarsInsets.top - systemBarsInsets.bottom)

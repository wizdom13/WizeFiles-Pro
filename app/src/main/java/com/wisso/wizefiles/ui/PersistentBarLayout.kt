...
import com.wisso.wizefiles.util.layoutInStatusBar
...
class PersistentBarLayout @JvmOverloads constructor(
    ...
    override fun dispatchApplyWindowInsets(insets: WindowInsets): WindowInsets {
        ...
        for (child in children) {
            if (isBarView(child)) {
                val childInsets = if (isTopBarView(child)) {
                    windowInsets.withSystemBarsInsets(
                        Insets.of(
                ...
                ViewCompat.dispatchApplyWindowInsets(child, childInsets)
        ...
        return windowInsets.withSystemBarsInsets(Insets.NONE).toWindowInsets() ?: insets
    }

    private fun updateContentViewsWindowInsets() {
        val windowInsets = lastInsets ?: return
        var contentSystemBarsInsets = windowInsets.getInsets(WindowInsetsCompat.Type.systemBars())
        for (child in children) {
            if (isTopBarView(child)) {
                ...
                contentSystemBarsInsets = Insets.of(
                    contentSystemBarsInsets.left,
                    (contentSystemBarsInsets.top - childConsumedInset).coerceAtLeast(0),
                    contentSystemBarsInsets.right,
                    contentSystemBarsInsets.bottom
                )
            }
        }
        val contentInsets = windowInsets.withSystemBarsInsets(contentSystemBarsInsets)
        for (child in children) {
    ...
    private fun WindowInsetsCompat.withSystemBarsInsets(systemBarsInsets: Insets): WindowInsetsCompat =
    ...
    private fun computeBarViewTop(barView: View): Int {
        ...
        return if (isTopBarView(barView)) {
            ...
            (measuredHeight + childLayoutParams.topMargin
                - (childRange * childLayoutParams.offset).toInt())
        }
    }
    ...
    fun hideBar(barView: View, animate: Boolean = true) {
        ...
        if (!isLaidOut) {
            ...
            if (isTopBarView(barView)) {
                ...
                bottomDragger.smoothSlideViewTo(
                    barView, barView.left, height + childLayoutParams.topMargin
                )
            }
        } else {
            childLayoutParams.offset = 0f
    ...
    private inner class ViewDragCallback(private val gravity: Int) : ViewDragHelper.Callback() {
        ...
        override fun clampViewPositionVertical(child: View, top: Int, dy: Int): Int =
            if (isTopBarView(child)) {
                ...
                val childLayoutParams = child.layoutParams as LayoutParams
                val shownTop = height - child.height - childLayoutParams.bottomMargin
                val hiddenTop = height + childLayoutParams.topMargin
                top.coerceIn(shownTop..hiddenTop)
            }
    }

...
import com.wisso.wizefiles.feature.apksigning.AabSigningRequest
...
import com.wisso.wizefiles.feature.apksigning.JarAabSigningBackend
...
internal class AabSigningFileOperationJob(
    ...
    override fun run() {
        ...
        val staging = File(application.cacheDir, "aab-signing/$operationId")
        staging.deleteRecursively()
        check(staging.mkdirs()) { "Unable to create private AAB signing staging directory" }
        var transferred = 0L
        try {
            secrets.use {
                val localSource = File(staging, "input.aab")
                ...
                val localOutput = File(staging, "signed.aab")
                JarAabSigningBackend().sign(
                    AabSigningRequest(localSource, localOutput, keyMaterial)
                )
            ...
            AabSigningOperationStore.delete(operationId)
...
internal class AabSigningSecretRequiredException : IOException(
    "AAB signing password must be entered again"
)

...
data class XapkSigningWorkflowSpec(
    ...
    init {
        ...
        require(sourceUri.isNotBlank()) { "An input XAPK is required" }
        require(outputUri.isNotBlank()) { "An output XAPK is required" }
        require(signingKeyUri.isNotBlank()) { "Signing key material is required" }
        require(sourceUri != outputUri) { "The original XAPK cannot be overwritten" }
        require(ApkSignatureScheme.V4 !in schemes) {
            "XAPK cannot contain detached v4 signature sidecars"
        }
        signatureSelection()

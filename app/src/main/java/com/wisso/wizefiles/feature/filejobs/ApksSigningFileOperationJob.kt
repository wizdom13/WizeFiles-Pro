...
import com.wisso.wizefiles.feature.apksigning.ApksArchiveSigningBackend
import com.wisso.wizefiles.feature.apksigning.ApksSigningOperationStore
import com.wisso.wizefiles.feature.apksigning.ApksSigningRequest
...
internal class ApksSigningFileOperationJob(
    operationId: String
) : FileOperationJob(operationId) {
    override fun run() {
        val operationId = requireNotNull(transferId)
        val spec = ApksSigningOperationStore.load(operationId)
            ?: throw IOException("APKS signing operation metadata is unavailable")
        val secrets = ApkSigningSecretRegistry.take(operationId)
            ?: throw ApksSigningSecretRequiredException()
        ...
        val staging = File(application.cacheDir, "apks-signing/$operationId")
        staging.deleteRecursively()
        check(staging.mkdirs()) { "Unable to create private APKS signing staging directory" }
        var transferred = 0L
        try {
            secrets.use {
                val localSource = File(staging, "input.apks")
            ...
            ApksSigningOperationStore.delete(operationId)
...
internal class ApksSigningSecretRequiredException : IOException(
    "APKS signing password must be entered again"
)

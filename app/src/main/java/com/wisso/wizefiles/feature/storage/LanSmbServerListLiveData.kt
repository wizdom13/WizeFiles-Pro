...
import com.wisso.wizefiles.util.getLocalAddress
import com.wisso.wizefiles.util.toLinkedSet
...
class LanSmbServerListLiveData : CloseableLiveData<Stateful<List<LanSmbServer>>>() {
    ...
    fun loadValue() {
        ...
        loadFuture = backgroundExecutor.submit {
            try {
                ...
                Executors.newFixedThreadPool(LanSmbDiscoveryPolicy.MAX_CONCURRENT_PROBES).asCoroutineDispatcher().use { dispatcher ->
                ...
                // Closing or reloading discovery is not a user-visible failure.
            } catch (e: Exception) {
    ...
    private fun Inet4Address.getSubnetAddresses(): Sequence<Inet4Address> =
        sequence {
            ...
            for (lastOctet in LanSmbDiscoveryPolicy.hostOctets()) {
                addressBytes[3] = lastOctet.toByte()
